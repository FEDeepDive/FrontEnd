...
ES5 이전의 JavaScript는 일부 잘못된 코드 때문에 스크립트 전체의 실행이 멈추는 것을 막기 위해, 발생하는 에러들을 조용히 무시하는 등 다소 ‘느슨한(sloppy)’ 방식으로 동작했다. ES5가 제정되며 도입된 ‘엄격 모드(strict mode)’를 적용하기 위해, `"use strict";` 구문을 사용한다.
