...
- **실행 (Execution)**
  ...
  - 실제로 변수에 값이 할당되는 단계
  - 비동기 관련 실행 과정은 [이벤트 루프, Web APIs, 콜백 큐 (비동기 처리)](#2-이벤트-루프) 참고
- **최적화 (Optimization)**
...
- **Web API에 의해 생성된 비동기 작업은 백그라운드에서 멀티 스레드로 처리됨**
  - 예시: `setTimeout(callback, 0)` 호출 시 비동기 작업(0ms 대기)이 생성됨 ⇒ 백그라운드에서 0ms 대기 ⇒ 타이머 만료 시 웹 브라우저 타이머 시스템에 의해 `callback`이 `콜백큐`에 추가됨
  ```
...
- 스코프와 스코프 체인 생성 ([스코프, 스코프 체인](#5-스코프))
- 변수 객체 생성 + 호이스팅 ([호이스팅(hoisting)](#3-호이스팅hoisting에-대해-설명해주세요))
