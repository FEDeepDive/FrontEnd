...
```
💡 선택자 우선순위는 “인라인 스타일 > ID 선택자 > Class  선택자 > 태그 선택자” 순으로 적용됩니다.
```
...
- **스타일 우선순위**
  ...
  - **`!important` 키워드**
    ...
    - 인위적으로 명시도를 높임
    - 권장되지 않음
  - **사용자 에이전트 스타일 시트(User Agent Stylesheet)**
    - 브라우저의 기본 스타일
  - **명시도(Specificity)**

    ```
    💡 인라인 스타일 > ID 선택자 > Class 및 속성 선택자 > 태그 선택자
    ```

    - 인라인 스타일(`style` 속성으로 HTML 요소에 직접 적용한 스타일)
    - ID 선택자(`#myElement`)
    - Class 선택자(`.myClass`), 속성 선택자(`[type="text"]`), 가상 클래스(`:hover`)
    - 유형 선택자(`div`), 가상 요소(`::before`)

    ```
    💡 명시도 계산 방법
    - 요소에 해당하는 스타일 개수 위 항목에 따라 각각 셈
    - (ex. 인라인 스타일 0개, ID 선택자 1개, Class 선택자 2개, 태그 선택자 0개)
    - 이를 숫자로 환산했을 때(ex. 0120 ⇒ 120) 가장 큰 숫자인 스타일을 적용
    ```
...
- **스타일 적용 측면**
  ...
  - **동적 및 반응형 디자인**
    ...
    - [변수 집합을 정의하여 테마를 간편하게 변경 가능](https://www.daleseo.com/css-variables/) (라이트 모드/다크 모드 등)
- **기능 측면**
  - **계산 및 표현식**
    - 변수를 활용해 스타일 내에서 동적으로 계산 가능
  - **스코핑**
    - 글로벌/로컬 스코핑으로 스타일 구조화 & 유연성 제공

## 15. 쌓임 맥락(Stacking Context)이란?

```
💡 쌓임 맥락은 Z축에서 요소의 위치를 나타냅니다. 쌓임 맥락은 `z-index`와 요소의 특성이나 요소 간 관계(부모-자손, 형제) 등에 의해 결정됩니다.
```

- **개념**
  - 문서를 가상의 Z축을 사용한 3차원 공간처럼 렌더링할 때, Z축 위에서의 요소의 위치(=앞/뒤)를 결정하는 방식
- **쌓임 맥락 생성 조건**
  - `z-index` 값이 유효한 요소
    - `z-index`가 큰 값이 위(앞)에 쌓임
    - 속성값이 `auto`인 경우(default) 새로운 쌓임 맥락을 생성하지 X
    - 설정되지 않은 경우 부모 요소의 Z축 위치와 동일한 위치에 존재
  - root 요소 (=문서는 기본적으로 쌓임 맥락을 가진다는 뜻)
  - `z-index` 유효 + `position` 속성이 `relative`, `absolute`, `fixed`, `sticky`인 요소
  - `z-index` 유효 + `display` 속성이 `flex`, `grid`인 요소의 자식 요소
  - `opacity`가 1보다 작은 요소
  - `float` 속성이 적용된 요소 (참고: [floating 엘리먼트의 쌓임](https://developer.mozilla.org/ko/docs/Web/CSS/CSS_positioned_layout/Understanding_z-index/Stacking_floating_elements))
- 주의할 점
  - 모든 요소가 쌓임 맥락을 생성하진 X
  - 쌓임 맥락은 계층 구조를 이룸
  - [자손 쌓임 맥락은 부모 쌓임 맥락 안에서 처리됨](https://developer.mozilla.org/ko/docs/Web/CSS/CSS_positioned_layout/Understanding_z-index/Stacking_context#%EC%98%88%EC%A0%9C)
    - ![position과 z-index로 인한 쌓임](./images/CSS/z-index.png) (position과 z-index로 인한 쌓임)
    - `DIV #4`, `DIV #5`, `DIV #6`은 `DIV #3`의 자식이므로, `DIV #3` 안에서만 쌓임이 처리됨
    - ⇒ `z-index` 속성값이 `DIV #4`>`DIV #1` 지만, `DIV #4`의 `z-index`는 `DIV #3` 안에서만 유효하므로 `DIV #4`가 더 뒤에 위치함
    - **TIP!** 부모-자손의 `z-index` 값을 버전 표기처럼 나타내면 됨
      ```
      - 루트
      	- DIV #2: (z-index: 2) => 2
      	- DIV #3: (z-index: 4) => 4
